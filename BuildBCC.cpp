...
int main(int argc, char* argv[])
{
    ...
    if (argc != 6 && argc != 7)
    {
        while ((opt = getopt_long(argc, argv, "h", long_option, &index)) != 1)
        {
            switch (opt)
            {
                case 'h':
                    printf ("Please enter CubicSize, LatticeSpacing, x0, y0, z0 following this order\n");
                    return 0;
                    break;
                case '?':
    ...
    {
        CubicSize = atof(argv[1]);
        LatticeSpacing = atof(argv[2]);
        x_0 = atof(argv[3]);
        y_0 = atof(argv[4]);
        z_0 = atof(argv[5]);
        if (argc == 6)
    ...
    vector<vector<double> > Atoms(height, vector<double>(width));
